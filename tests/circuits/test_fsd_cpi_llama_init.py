...
def test_cpi_attention_init_runs_and_is_finite():
    ...
    import substill
    from substill.compression.cpi import apply_cpi_attention_init

    loader = _loader()
    from substill.compression.cpi import cpi_rank_map

    pipe = substill.FSDPipeline(teacher, config=substill.FSDConfig(template="llama"))
...
def test_ov_align_init_runs_and_preserves_forward_finiteness():
    ...
    if teacher is None:
        pytest.skip("transformers not installed")
    import substill
    from substill.compression.cpi import apply_ov_align_init, cpi_rank_map
...
def test_cpi_attention_init_both_rope_modes():
    ...
    if teacher is None:
        pytest.skip("transformers not installed")
    import substill
    from substill.compression.cpi import apply_cpi_attention_init, cpi_rank_map
