"""FSDPipeline + CPSD-factored conversion on a real (tiny) GPT-2 (task #10).
...
def test_cpsd_conversion_preserves_forward_then_trains():
    ...
    pipe = fasd.FSDPipeline(teacher, config=fasd.FSDConfig(arch_multiplier=0.5))
    pipe.run_profile(_loader())
    student = pipe.build()
    ...
    n = fasd.convert_gpt2_to_factored(student, teacher, pipe.profile)
