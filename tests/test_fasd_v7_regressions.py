"""Regressions caught by the v7-apr24 ablation run.

The per-linear absorbed_init math is correct (existing tests cover it), but
the *composed* student forward was catastrophically broken — absorbed-init
students (v7 rungs 4–7) started at PPL 10^10 to 10^15, because:
...
def test_absorbed_init_composed_student_ppl_is_sane():
    ...
    v7-apr24 rungs 4–7 saw initial PPL 10^10 to 10^15. With the Q/K-shared-basis
    fix and orthogonal (not zero) column padding, the composed forward is
...
def test_procrustes_loss_magnitude_comparable_to_cka():
    ...
    v7-apr24 rungs 2–3 diverged when the loss schedule hit frac=0.4 (cka→procrustes)
    because the unnormalized procrustes cost was O(k) vs CKA's O(1). After
    ...
    """
    from fasd.losses.procrustes import procrustes_distance
    from fasd.losses.subspace import cka_distance
...
def test_absorbed_init_orthogonal_col_padding():
    ...
    """
    from fasd.builders import _pad_cols_orthogonal
...
def test_refresh_from_profile_preserves_folded_shape():
    ...
    v8-apr24 rungs 5 and 7 crashed at the on-policy profile refresh: a new PCA
    computed on student rollouts returned ``behavioral_rank=2664`` for
    ``ffn.up``, but the student's intermediate dim was 768 and the projector
    had been folded. A blind basis swap broke the next forward. The refresh
    now keeps the folded rank locked and only updates the basis where shape
    allows.
    """
    from fasd.losses.subspace import F_ASDLoss
    ...
    # Refresh with a LARGER behavioral rank (the crash case in v8).
...
# -- v9-apr25 regressions ---------------------------------------------


def test_col_basis_uses_pca_tail_when_under_ranked():
    """``_col_basis`` should slice PCA tail directions, not random-pad.

    Previously when behavioral_rank < cols_needed the code zero-padded (v7)
    or randomly padded (v8). With principal_components stored as the full
    ...
    """
    from fasd.builders import _col_basis
...
def test_quantized_linear_fp_weight_receives_grad():
    ...
    v8 froze quantized weights as int8 buffers with no STE — qad_finetune was
    powerless against the quantization error. Now fp_weight is a Parameter and
    fake-quant has STE, so backprop flows through.
    """
    from fasd.compression.quantization import QuantizedLinear
...
def test_residual_basis_prefers_residual_branch():
    ...
    v9-G: when a profile includes a true residual-stream PCA branch, the
    absorbed_init should use it directly rather than averaging attn.o + ffn.down.
    """
    from fasd.builders import _residual_basis
...
def test_residual_basis_falls_back_when_no_residual_branch():
    ...
    v10 dropped the v8/v9 attn.o+ffn.down averaged-then-QR fallback (it had no
    theoretical justification and added noise). The new fallback is a truncated
...
def test_residual_basis_identity_when_no_compression():
    """v10: when ``s_hidden == t_hidden``, ``_residual_basis`` returns the identity.

    v9 returned a PCA-derived orthogonal rotation in this case, which broke
    initial PPL by 5–14 orders of magnitude on rung 4 because LayerNorm does
...
def test_channel_select_basis_picks_top_variance_channels():
    """v10: ``_channel_select_basis`` returns one-hot columns at the top channels
    ...
    """
    from fasd.builders import _channel_select_basis
...
def test_gpt2_absorb_full_size_reproduces_teacher():
    """v10: with no residual or FFN compression, absorbed init reproduces the
    teacher exactly (modulo floating point).

    This test would fail under v9: even with k=d=t_hidden, V_r was a non-trivial
    orthogonal rotation from PCA, breaking LayerNorm and per-head attention. v10
    short-circuits to V_r = I, V_up = I in that case.
    """
