...
def test_llama_cpsd_conversion_preserves_forward_gqa():
    ...
    n = substill.convert_llama_to_factored(student, teacher, pipe.profile, free_core=False)
    assert n > 0
...
def test_llama_cpsd_free_core_trains():
    ...
    pipe.run_profile(_loader())
    student = pipe.build()
    substill.convert_llama_to_factored(student, teacher, pipe.profile, free_core=True)
    ...
    loss = substill.forward_kl(s_logits, t_logits)
    opt.zero_grad()
    loss.backward()
    opt.step()
