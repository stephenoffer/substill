...
#
# Each job runs scripts/cpsd_compare.py for ONE (arch-multiplier x seed) cell on
...
#
# Usage:
#   TAG=cmp-v1 bash scripts/cpsd_compare_submit.sh
#   CANARY=1 TAG=cmp-canary bash scripts/cpsd_compare_submit.sh   # 1 quick job
set -eo pipefail
  ...
  for S in "${SEED_ARR[@]}"; do
    ...
    anyscale job submit \
      ...
      -- bash -c "pip install --quiet transformers==5.6.1 datasets==3.6.0 tqdm && \
        PYTHONPATH=. python scripts/cpsd_compare.py \
...
echo "Aggregate (after sync): python scripts/cpsd_aggregate.py results/*.json"
