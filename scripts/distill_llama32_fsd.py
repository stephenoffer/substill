...
Composes all Sprint 1-6 pillars:
  - γ-fold pre-pass (Pillar 1, half a)
  - RR-Norm replacement in student (Pillar 1, half b)
  - Function-aware Fisher scoring (Sprint 3)
  - Exact greedy q/cost rank allocator (Sprint 3)
  - Block-diagonal sparse correction (Pillar 3 / Sprint 4)
  - Trainable Stiefel bases via StiefelAdam (Pillar 2 / Sprint 5)
  - Adaptive entropy-gap skew-KL + plateau detector + unified token weighting (Sprint 6)
...
Compute requirements:
  ...
  - Token budgets: 5B / 10B / 20B per Sprint 7's data-efficiency sweep
...
from fasd import build_student
from fasd.api import profile as fasd_profile
from fasd.compression.rank_allocator import allocate_ranks, edges_from_profile
from fasd.compression.sparse_block import CorrectedLinear
...
# --- FSD pillars --------------------------------------------------------------
from fasd.profiling.gamma_fold import fold_gpt2, fold_llama
from fasd.training.stiefel_optim import StiefelAdam, stiefel_param_groups
from fasd.util.param_accounting import breakdown
from fasd.util.rr_norm import replace_layernorm_with_rrnorm
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--calib-sequences", type=int, default=4096,
                   help="Sequences for streaming PCA + Fisher scoring (Sprint 3 scale).")
    ...
    p.add_argument("--use-rr-norm", action="store_true",
                   help="Replace student norms with RR-Norm + Stiefel Q (Pillar 1).")
    p.add_argument("--use-fisher-score", action="store_true",
                   help="Score directions by Fisher (Sprint 3) vs. KL-patch baseline.")
    p.add_argument("--use-exact-allocator", action="store_true",
                   help="Use greedy q/cost knapsack allocator (Sprint 3).")
    p.add_argument("--use-sparse-block", action="store_true",
                   help="Add block-diagonal per-head correction (Pillar 3).")
    p.add_argument("--use-stiefel-optim", action="store_true",
                   help="Train Q (and bases) on Stiefel via StiefelAdam (Pillar 2).")
    p.add_argument("--use-adaptive-skew-kl", action="store_true",
                   help="Per-token entropy-gap skew-KL (Sprint 6).")
    p.add_argument("--use-plateau-trigger", action="store_true",
                   help="Trigger on-policy ramp from plateau detection (Sprint 6).")
...
def stage_a_gamma_fold(teacher, args):
    """Pillar 1, half a: γ-fold the teacher.
...
def stage_c_score_directions(folded_teacher, profile, calib_loader, args):
    """Sprint 3: Fisher-weighted directional scoring.
...
def stage_d_allocate_ranks(profile, direction_scores, target_params, args):
    """Sprint 3: greedy q/cost knapsack allocator under exact param budget.
    ...
    *relative* cost ordering correct.
    ...
    def step_fn(b):
        return 1  # default; refine to head-group units in Sprint 2 follow-up
...
def stage_e_build_student(teacher, profile, allocation_result, args):
    """Build the student via the existing builder, then apply Pillar 1/3 modifications.

    When the exact allocator (Sprint 3) ran, its rank-map is passed directly to
    ``build_student`` via the ``rank_map=`` kwarg, which overrides each branch's
    stored ``behavioral_rank`` without going through ``arch_multiplier``. This
    closes the integration TODO #1 from HANDOFF.md.
    """
    ...
    if args.use_rr_norm:
        ...
        print(f"[fsd] Pillar 1: replaced {n_replaced} norm layers with RR-Norm")

    if args.use_sparse_block:
        n_injected = inject_sparse_blocks(student)
        print(f"[fsd] Pillar 3: injected block-diagonal correction into {n_injected} linears")
...
def inject_sparse_blocks(student: nn.Module) -> int:
    ...
    output (Pillar 3's correction grows during training).
...
def stage_f_train(student, teacher, train_loader, args):
    """Distillation training loop with Pillar 2 (Stiefel) + adaptive objective."""
    ...
    if args.use_stiefel_optim:
        ...
        print(f"[fsd] Pillar 2: StiefelAdam with {sum(1 for g in groups if g.get('stiefel'))} "
