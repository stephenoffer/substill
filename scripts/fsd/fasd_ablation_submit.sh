...
# Each job runs one rung of scripts/fsd/fasd_ablation.py on one A10G.
#
...
for SPEC in "${JOBS[@]}"; do
  ...
  anyscale job submit \
    ...
    --exclude '.pytest_cache' --exclude '*.profile' --exclude '*.substill' \
    --env "FASD_TAG=${TAG}" \
    --env "FASD_RUNG=${RUNG}" \
    ${EXTRA_ENV} \
    --env "HF_HOME=/mnt/cluster_storage/hf" \
    --env "PYTHONUNBUFFERED=1" \
    -- bash -c "pip install --quiet transformers==5.6.1 datasets==3.6.0 tqdm && python scripts/fsd/fasd_ablation.py --rung '${RUNG}' --tag '${TAG}' --teacher ${TEACHER} --batch-size 4 --seq-len 128 --total-steps ${TOTAL_STEPS} --rank-tol 0.02 --max-rank ${MAX_RANK} --calib-batches 32 ${EXTRA_ARGS}"
done

echo
echo "All jobs submitted."
echo "Monitor:  anyscale job list --include-all-users | grep substill-${TAG}"
echo "Results:  aws s3 ls \$ANYSCALE_ARTIFACT_STORAGE/substill/${TAG}/results/"
