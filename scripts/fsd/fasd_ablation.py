...
def _resolve_output_dir(tag: str) -> str:
    ...
    if base:
        return os.path.join(base, "substill", tag)
    return os.path.join(os.getcwd(), "outputs", "substill", tag)
...
def main():
    ...
    print(f"[substill-ablation] rung={args.rung} tag={args.tag} config={config}")
    ...
    print(f"[substill-ablation] output_dir={output_dir}")

    if _path_exists(result_path):
        print(f"[substill-ablation] skipping — result already at {result_path}")
...
def _find_arch_multiplier(teacher, profile, target_compression: float) -> float:
    ...
    print(
        f"[substill-ablation] matched-compression search: target={target_compression:.2f}x "
...
def _stage(label: str, t0_holder: list) -> None:
    ...
    print(f"[substill-stage] {label} (+{elapsed:.1f}s)", flush=True)
    t0_holder[:] = [now]
...
def _run_rung(args, config, result_path: str) -> None:
    ...
    if not config["use_behavioral"]:
        ...
        profile = substill.TeacherProfile(branches=new_branches, meta=profile.meta)
    ...
    print(
        f"[substill-ablation] teacher_params={teacher_params/1e6:.1f}M  "
    ...
    if config["objective"] == "schedule":
        schedule = substill.default_schedule()
    ...
    print(f"[substill-ablation-result] {json.dumps(summary)}")
