...
#
# Each job runs scripts/fsd_headline_experiment.py for ONE (compression x seed) cell,
...
#
# Usage:
#   TAG=cpsd-v1 bash scripts/cpsd_experiment_submit.sh
#   CANARY=1 TAG=cpsd-canary bash scripts/cpsd_experiment_submit.sh   # 1 quick job
set -eo pipefail
  ...
  for S in "${SEED_ARR[@]}"; do
    ...
    anyscale job submit \
      ...
      -- bash -c "pip install --quiet transformers==5.6.1 datasets==3.6.0 tqdm && \
        PYTHONPATH=. python scripts/fsd_headline_experiment.py \
