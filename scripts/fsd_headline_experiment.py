...
compression and matched compute:
  ...
  - **r3_fsd**:     absorbed-init + RR-Norm (Pillar 1) + sparse-block correction
                    (Pillar 3) + adaptive entropy-gap skew-KL (Sprint 6)
...
from fasd.compression.diff_rank import DifferentiableRankGate, RankBudgetController
from fasd.compression.factored_linear import TeacherFactoredLinear
from fasd.losses.generative_kd import adaptive_skew_kl, forward_kl
from fasd.pipeline import convert_gpt2_to_factored
from fasd.profiling.gamma_fold import fold_gpt2
from fasd.util.param_accounting import count_params
...
def build_fsd_student(teacher, train_ids, args):
    ...
    Pillar 1 requires that the teacher's LayerNorm γ/β are folded into the
    next linear *before* profiling. After folding, the teacher's LN modules
...
def train_variant(variant, teacher, train_ids, val_ids, profile, args):
    ...
    if variant in ("r0_random", "r1_kd_random"):
        ...
        # Use StiefelAdam to keep RR-Norm's Q matrices on the manifold (Pillar 2).
