...
def load(args):
    if args.smoke or args.teacher == "tiny":
        ...
        torch.manual_seed(0)
        mk = lambda n: [{"input_ids": (t := torch.randint(5, 60, (args.batch_size, args.seq_len))),
                         "labels": t} for _ in range(n)]
...
VARIANTS = {
    "r1_kd_random": dict(absorbed_init=False),
    "r2_fasd":      dict(absorbed_init=True),
    "cpsd_mt":      dict(use_cpsd_factored=True),
    "cpsd_full":    dict(use_cpsd_factored=True, use_diff_rank=True),
    "dobi_svd":     dict(use_cpsd_factored=True, use_diff_rank=True),
}
...
def _make_pipe(name, teacher, profile, calib, args, total_steps):
    ...
    pipe = fasd.FSDPipeline(teacher, config=config)
...
def run_variant(name, teacher, profile, calib, train, val, args):
    from fasd.training.distill import distill
...
def main():
    ...
    profile = fasd.profile(teacher, train[: args.calib_batches])
    ...
    for name in args.variants:
        try:
            r = run_variant(name, teacher, profile, train[: args.calib_batches],
                            train, val, args)
            print(f"[compare] {name:<14} PPL={r['final_ppl']:.2f}  ratio={r['compression_ratio']:.2f}x")
        except Exception as e:  # noqa: BLE001
