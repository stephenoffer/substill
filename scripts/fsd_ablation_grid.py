"""Ablation grid driver for FSD pillars.

Runs every combination of pillars on/off at a fixed compression and budget,
emits a single CSV summarising the contribution of each pillar.

Pillars (each toggle is one column in the ablation grid):
  - --use-rr-norm           (Pillar 1: rotation-equivariant normalization)
  - --use-fisher-score      (Sprint 3: Fisher-weighted scoring)
  - --use-exact-allocator   (Sprint 3: greedy q/cost knapsack)
  - --use-sparse-block      (Pillar 3: block-diagonal correction)
  - --use-stiefel-optim     (Pillar 2: trainable Stiefel bases)
  - --use-adaptive-skew-kl  (Sprint 6: adaptive entropy-gap skew-KL)
  - --use-plateau-trigger   (Sprint 6: plateau-driven on-policy ramp)
