...
#
# Each job runs scripts/vision/resnet50_distill.py for ONE width-ratio on CIFAR-10: it warms up the
...
#
# Usage:
#   TAG=rn50-v1 bash scripts/vision/resnet50_distill_submit.sh
#   CANARY=1 TAG=rn50-canary bash scripts/vision/resnet50_distill_submit.sh
set -eo pipefail
...
for R in "${RATIO_ARR[@]}"; do
  ...
  anyscale job submit \
    ...
    -- bash -c "pip install --quiet torchvision tqdm && \
      PYTHONPATH=. python scripts/vision/resnet50_distill.py \
