...
:func:`substill.compression.absorbed_init.absorbed_weight`), and the KD losses (which operate
...
def top1_accuracy(model: nn.Module, loader: Iterable, *, device=None,
    ...
    """Compute top-1 classification accuracy of ``model`` over ``loader``."""
    if device is None:
        device = next(model.parameters()).device
    model.eval().to(device)
    correct = total = 0
    ...
    while step < total_steps:
        for batch in train_loader:
            ...
            if log_every and step % log_every == 0:
                print(f"[substill.vision] step={step}/{total_steps} kd={float(kd.item()):.4f}",
                      flush=True)
            step += 1
    out = {"history": history}
