...
def _group_fake_quant(W: Tensor, bits: int, group_size: int) -> Tensor:
    ...
    weights. Without the STE, the quantized weights would be a frozen buffer
    and ``qad_finetune`` would only update bias / protected / LN, leaving the
    99% quantized bulk inert.
    """
    if bits < 2 or bits > 8:
        raise ValueError(f"bits must be in [2, 8], got {bits}")
    if group_size < 1:
...
class QuantizedLinear(nn.Module):
    ...
    An int8 ``q_weight`` stored as a frozen buffer with no STE would let QAD
    move only bias / protected / LN parameters, making 4-bit quantization a
    steep PPL tax over the unquantized weight.
    """
