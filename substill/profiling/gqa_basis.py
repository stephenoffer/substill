"""GQA shared-per-KV-group basis (fix for the disjoint-basis bug).

The bug
-------

The current absorbed-init code at [substill/builders.py:477-485](../builders.py#L477-L485)
...
is **false** and was empirically refuted: an arbitrary cross-plane PCA basis
inflates the *post-RoPE* QK score error ~7×.
...
planes. Therefore:
  ...
  - **QK projections (RoPE):** use the RoPE-aware *plane-aligned* basis in
    :mod:`substill.profiling.rope` (``rope_aware_basis``), not the free cross-plane
...
::
    ...
    from substill.profiling.gqa_basis import collect_gqa_bases
...
The builder integration is a follow-up; this module supplies the math + tests.
"""
