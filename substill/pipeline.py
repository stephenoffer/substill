...
:func:`substill.profile`, :func:`substill.build_student`, :func:`substill.distill` — and threads
...
CPSD post-build conversions (all optional, off by default):
  - ``use_cpsd_factored``: replace absorbed linears with :class:`TeacherFactoredLinear`
    ...
    (the manifold-training novelty). Use :func:`substill.training.stiefel_optim.StiefelAdam`.
  - ``use_rr_norm``: replace norms with rotation-equivariant ``RRNorm``.
"""
...
def convert_gpt2_to_factored(
    ...
    *,
    ...
    Uses :func:`substill.builders.gpt2_absorb_targets` to recover the exact
    ...
    the free core supplies the missing fitting capacity while
...
def convert_llama_to_factored(
    ...
    *,
    ...
    drops in directly. Uses :func:`substill.builders.llama_absorb_targets` to recover each
...
def attach_diff_rank_gates(
    ...
    *,
    ...
    """Wrap every factored edge with a gate and return a DDR budget controller.

    Each :class:`TeacherFactoredLinear` in ``student`` is wrapped with a
    :class:`GatedFactoredLinear`; the returned :class:`RankBudgetController` drives
    the gates. The controller's budget is ``target_ratio`` × the factored edges'
...
class FSDPipeline:
    ...
    def fold_for_inference(self, *, harden: bool = True):
        """Collapse all factored/gated edges back to plain ``nn.Linear`` for inference.

        Returns the hardened DDR rank-map (edge name -> integer rank) when DDR was
        used, else ``{}``. Call after :meth:`train`.
        """
        if self.student is None:
