...
def _resolve_fold_frac(
    ...
    rotation at the ``frac >= 0.10`` boundary.
    """
...
def distill(
    ...
    *,
    ...
    if progressive_stages > 1 and hasattr(student, "config"):
        ...
        # Records the plan but does not instantiate intermediate models
    ...
    while step < total_steps:
        ...
        if feature_cache is not None:
            ...
            pass  # caching path reuses the teacher anyway for simplicity
        ...
        if not torch.isfinite(total):
            ...
            if consecutive_nan_trips >= 2:
                ...
                print(
                    f"[substill.distill] nan_abort step={step} after 2 nonfinite losses",
                    flush=True,
                )
                break
        ...
        if log_every > 0 and (step % log_every == 0 or step == total_steps - 1):
            print(
                f"[substill.distill] step={step}/{total_steps} frac={frac:.2f} "
        ...
        # rotation into the teacher V buffer; an earlier frac>=0.10 threshold gave
        # the projector only ~10% of training to converge before being absorbed.
        ...
        if (
            ...
            try:
                ...
                profile = new_profile
                print(
                    f"[substill.distill] reabsorb step={step} frac={frac:.2f} "
                ...
                print(f"[substill.distill] reabsorb failed at step {step}: {e}", flush=True)
