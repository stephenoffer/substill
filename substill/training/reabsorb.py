...
aligned bases go stale. Adding mechanisms on top of static absorption can
make things worse, not better: both on-policy and full-compression runs
have been observed to regress in PPL rather than improve.
...
def _rotate_optim_state_for(
    ...
    - Adam's update is ``lr * m_hat / (sqrt(v_hat) + eps)``. After PRA we
      ...
      ``m / eps`` — an effective learning-rate multiplier of ~10^8. This
      catastrophic blowup was confirmed empirically.
...
def reabsorb_gpt2(
    ...
    *,
    ...
    # which is the failure mode that made training diverge.
    for p in (wte, wpe):
