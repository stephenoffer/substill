...
After γ-fold (see [substill/profiling/gamma_fold.py](../profiling/gamma_fold.py)),
...
2. **Learnable orthogonal correction ``Q ∈ O(d_S)``** — a Stiefel-trainable
   ...
   ``I``. The Stiefel optimizer ([substill/training/stiefel_optim.py](../training/stiefel_optim.py))
...
def replace_layernorm_with_rrnorm(
    ...
    *,
    ...
    (a catastrophic init blow-up). The user must run γ-fold *first* (see
    :mod:`substill.profiling.gamma_fold`); norms that cannot be folded (e.g. GPT-2's
