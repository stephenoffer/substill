...
:func:`substill.compression.width_pruner.profile_to_student_config`),
instantiates a fresh student, and — when ``absorbed_init=True`` —
fills its linear weights using
:func:`substill.compression.absorbed_init.absorbed_linear_init`.
...
def _residual_basis(profile, t_hidden: int, s_hidden: int) -> Tensor:
    ...
    When ``s_hidden == t_hidden`` (no residual compression) we
    return the identity. Applying a PCA rotation here even when no
    compression is happening breaks initial PPL by
    ...
    There is deliberately no ``attn.o``/``ffn.down`` averaged-then-QR
    fallback: it has no theoretical justification and makes things worse.
    """
    ...
    if cov_diag_sum is not None:
        ...
        # the expected semantics for tests that don't supply eigenvalues.
    ...
    # No residual capture: fall back to identity truncation (keep the first
    # ``s_hidden`` coordinates).
    #
    # This path is the DEFAULT, not an edge case: ``profile()`` runs with
    # ``mode="branch"``, which emits no ``block.residual`` branch at all, so the
    # loop above never finds one and every absorbed-init student built through
    # ``FSDPipeline`` has silently used identity truncation. Verified by
    # ``tests/test_seq_absorb.py::test_profile_default_yields_identity_residual_basis``.
    #
    # We keep identity truncation as the behavior because it measured best of the
    # bases we tried: after matched distillation it beats variance-ranked channel
    # selection, while a PCA rotation diverged. But it must not arrive silently,
    # because it means the profile's residual statistics are unused.
    warnings.warn(
        "_residual_basis: no 'block.residual' branch in the profile, so the "
        "residual stream is being truncated to its first "
        f"{s_hidden} of {t_hidden} coordinates. The profile's residual statistics "
        "are NOT being used. profile(mode='residual') captures them, but the "
        "statistics-driven bases measured worse than this fallback.",
        stacklevel=2,
    )
...
def _channel_select_basis(profile, branch_name: str, k: int) -> Tensor:
    ...
    inflated initial student PPL by 4 orders of magnitude.
    Channel-selection slices instead of rotating, so
...
def _pad_cols_orthogonal(V: Tensor, cols_needed: int) -> Tensor:
    ...
    where the student is expected to do something — an earlier basis choice
    caused absorbed-init students to start at PPL 10^10 to 10^15.
...
def _pad_rows_to(V: Tensor, rows: int) -> Tensor:
    """Pad or truncate ``V`` to ``rows`` rows (zero-pad below, slice above).

    Lifts a behavioral-rank basis back to the full teacher dimension; shared by
    the GPT-2 and Llama absorb paths.
    """
    if V.shape[0] == rows:
        return V
    if V.shape[0] > rows:
...
def _build_gpt2(teacher, profile, student_cfg: StudentConfig, absorbed_init: bool):
    try:
        ...
        raise ImportError(
            "substill.build_student template='gpt2' requires the 'transformers' package"
        ) from e
...
def gpt2_absorb_targets(teacher, student, profile):
    ...
    re-absorption (:mod:`substill.training.reabsorb`). The bases are derived
...
def _build_llama(teacher, profile, student_cfg: StudentConfig, absorbed_init: bool):
    try:
        ...
        raise ImportError(
            "substill.build_student template='llama' requires the 'transformers' package"
        ) from e
    ...
    for i, s_block in enumerate(s_layers):
        ...
        # channel-select for FFN intermediate. SiLU is element-wise so
...
def build_student(
    ...
    *,
    ...
    :func:`substill.compression.rank_allocator.allocate_ranks`. When provided,
    overrides each branch's stored ``behavioral_rank`` and disables
    ``arch_multiplier`` scaling — the rank-map already encodes the budget (and its
    head geometry, so ``preserve_head_dim`` is ignored).

    ``preserve_head_dim`` (default ``True``): size the student's ``hidden_size`` to a
    multiple of the teacher's head_dim and drop whole heads, rather than keeping the
    teacher's head count and shrinking head_dim. Absorbed init truncates the residual
    stream to its first ``hidden_size`` coordinates, and heads live contiguously along
    that axis, so this makes each surviving student head an intact teacher head. Worth
    ~7% final PPL at matched parameters; see
    :func:`substill.compression.width_pruner.profile_to_student_config`.
    """
    if template == "auto":
    ...
    cfg = profile_to_student_config(
        ...
        rank_map=rank_map,
        preserve_head_dim=preserve_head_dim,
    )
    if template == "gpt2":
