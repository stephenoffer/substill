...
The [top-level README](../README.md) is the pitch + API overview.
This directory has the technical docs.
...
directions per block (with a Marchenko-Pastur bulk-edge cutoff that
rejects noise-bulk eigenvalues). `asd.SubspaceLoss(profile)` is a
drop-in `nn.Module` loss that projects student hidden states into
the teacher's retained subspace and matches them with centered
kernel alignment (scale-invariant, best for LLMs), Gram-Frobenius
...
- **Understand what the method is doing mechanistically** →
  [algorithm.md](algorithm.md).
- **Skim the code** → [`asd/api.py`](../asd/api.py). Single file,
  ~700 lines, heavily commented.
- **Reproduce the benchmarks in the top-level README** →
  [`scripts/finetune_teacher.py`](../scripts/finetune_teacher.py) +
