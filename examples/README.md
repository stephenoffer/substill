...
Short, self-contained scripts that run on CPU in seconds and need no downloads —
they build tiny models from scratch. Install the package first (`pip install -e .`;
add `[llm]` for the GPT-2 examples and `torchvision` for the vision one).
