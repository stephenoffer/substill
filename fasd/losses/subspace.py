...
5. After warm-up, :meth:`fold_projectors_into_` converts each
   ...
   :func:`fasd.build_student` ensures by construction).
"""
...
class F_ASDLoss(nn.Module):
    ...
    def refresh_from_profile(self, profile) -> None:
        ...
        In v8-apr24 rungs 5 and 7 the on-policy refresh produced a new PCA with
        ``behavioral_rank=2664`` for ``ffn.up`` while the student's intermediate
        dim was 768; a blind swap broke the next forward pass.
        """
    ...
    def fold_projectors_into_(self, student: nn.Module) -> None:
        ...
        ``C_s == k`` — which :func:`fasd.build_student` ensures when
