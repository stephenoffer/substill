...
def cpi_rank_map(teacher, profile, *, head_dim_ratio: float = 0.5,
    ...
    """Build a rank-map pinning a **CPI-compatible** student: keep the teacher's
    attention-head count H and KV-group count G, compress only the per-head dim
    (and the FFN). CPI's shared-per-group basis requires H_s==H_t and G_s==G_t so
    the per-group block-diagonal mapping is well-defined (this avoids the harder
    KV-group-reduction case). ``head_dim_ratio`` sets s_d_h = round(d_h·ratio) (even).
    """
    cfg = teacher.config
...
def _accumulate_group_covariances(teacher, calib_batches, *, device=None):
    """Capture per-layer q/k/v (post-projection, pre-RoPE) and accumulate the joint
    per-KV-group covariance. Returns dict[layer_idx -> (G, d_h, d_h)]."""
...
def apply_ov_align_init(
    ...
    *,
    ...
    """Fix the OV circuit for FREE by aligning ``o_proj``'s input basis to ``v_proj``'s
    output basis (V's own per-group PCA) — no compromise/shared basis, no energy cost.
    ...
    if H != H_t or G != G_t:
        raise ValueError("apply_ov_align_init requires H_s==H_t and G_s==G_t "
                         "(use cpi_rank_map); got H_s=%d H_t=%d G_s=%d G_t=%d"
                         % (H, H_t, G, G_t))
    s_d_h = s_hidden // H
    ...
    for i, (tb, sb) in enumerate(zip(layers, student.model.layers)):
        ...
        Vo = torch.block_diag(*[v_basis[h // heads_per_group] for h in range(H)])  # (t_hidden, s_hidden)
...
def apply_cpi_attention_init(
    ...
    *,
    ...
    """Re-initialize a Llama-family student's attention projections with a
    **shared-per-KV-group circuit-preserving** basis, fixing the GQA disjoint-basis
    bug (``builders.py:483-485``).
    ...
    if H != H_t or G != G_t:
        raise ValueError(
            ...
            f"Build the student with fasd.compression.cpi.cpi_rank_map(...) as rank_map."
        )
    ...
    for i, (tb, sb) in enumerate(zip(t_layers, s_layers)):
        ...
        Vq = torch.block_diag(*[qk_basis[h // heads_per_group] for h in range(H)])  # (t_hidden, s_hidden)
        Vk = torch.block_diag(*[qk_basis[g] for g in range(G)])                     # (t_kv, s_kv)
        Vv = torch.block_diag(*[v_basis[g] for g in range(G)])                      # (t_kv, s_kv)
        Vo = torch.block_diag(*[v_basis[h // heads_per_group] for h in range(H)])   # (t_hidden, s_hidden)
