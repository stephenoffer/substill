"""Block-diagonal per-head sparse correction (Pillar 3).
    ...
    def warm_init_from_residual(
        ...
        *,
        ...
        if teacher_weight.shape != absorbed_weight.shape:
            ...
            # space and a same-shape "ideal" teacher reconstruction. For simplicity
            # we just reset to zero in this simplified API.
            self.weight.zero_()
            return
...
class CorrectedLinear(nn.Module):
    ...
    This is what the student's attention/FFN linears become after Pillar 3
    is applied. The base linear's weight is the absorbed-init projection; the
