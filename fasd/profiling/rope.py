...
----------------------
The shared-subspace QK construction in :mod:`fasd.profiling.gqa_basis` preserves the
...
and was empirically refuted (``runs/derisk/rope_circuit_basis.py``: an arbitrary PCA
basis inflates the post-RoPE score error ~7× vs the no-RoPE case).
...
:mod:`fasd.compression.cpi` — only the QK projections need this module.
"""
